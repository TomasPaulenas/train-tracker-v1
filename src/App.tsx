...
import {
  ...
  updateExerciseName,
  handleSetField,
} from "./lib/update/workoutsModel";
import { removeExerciseFromWorkout } from "./lib/update/removeExerciseFromWorkout";
import { updateWorkout } from "./lib/update/updateWorkout";
        ...
        return (
          <div key={workout.id}>
            <WorkoutHeader
              ...
              onFinishEdit={(title) => {
                setWorkouts((prev) =>
                  updateWorkout(prev, workout.id, { title })
                );
                setEditingWorkoutId(null);
              }}
            ...
            {isOpen && (
              <WorkoutBody
                ...
                onExerciseNameChange={(exerciseId, name) =>
                  setWorkouts((prev) =>
                    updateExerciseName(prev, workout.id, exerciseId, name)
                  )
                }
                onSetField={(exerciseId, field, value) =>
                  setWorkouts((prev) =>
                    handleSetField(prev, workout.id, exerciseId, field, value)
                  )
                }
              />
            )}
